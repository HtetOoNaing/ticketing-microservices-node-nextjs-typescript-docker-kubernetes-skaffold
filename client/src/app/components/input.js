export default function Input({type, id, placeholder, value, onChange, label, className = '', containerClassName = '', labelClassName = '' }) {
    return (
        <div className={`${containerClassName}`}>
            <label htmlFor={id} className={`block ml-1 mb-2 text-sm font-medium text-gray-700 ${labelClassName}`}>{label}</label>
